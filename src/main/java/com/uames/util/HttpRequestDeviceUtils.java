...
public class HttpRequestDeviceUtils {
    ...
    /**iPhone的User-Agent关键词*/
    private static String[] iPhoneHeaders=new String[]{
        "iPhone",
        "iPod",
        "iPad",
    };
    ...
    */
    ...
    public static boolean isIPhone(HttpServletRequest request){
        ...
        if(isMobileDevice(request)){
            for(int i = 0; userAgent!=null && !userAgent.trim().equals("") && i < iPhoneHeaders.length; i++){
                if(userAgent.contains(iPhoneHeaders[i])){
                    b = true;
                    break;
                }
            }
